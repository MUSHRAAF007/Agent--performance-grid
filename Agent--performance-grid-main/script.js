...
const CONFIG = {
  ...
  APPS_SCRIPT_URL: "https://script.google.com/macros/s/AKfycbwbBu5Lrc80uDr58TSZAKTpoiiHPJNiMrr2S4KgtbkV02-yCD4dglk8gj75aWMlfzxT/exec",
};
...
function renderAgentCard(agent) {
  return `
    <div class="agent-card p-3 sm:p-4 flex flex-col gap-3 sm:gap-4 cursor-pointer shadow-lg hover:shadow-2xl transition-all" onclick="openAgentModal('${agent.name}')">
      
      <!-- TOP SECTION: PROFILE -->
      <div class="flex items-center gap-2 sm:gap-3">
        <div class="relative shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/20 overflow-hidden border-2 border-white/40 shadow-md">
          ...
          <div class="hidden w-full h-full items-center justify-center bg-gradient-to-br from-theme-primary to-theme-secondary">
             <span class="text-white font-heading font-bold text-lg sm:text-xl drop-shadow-md">${agent.name.charAt(0).toUpperCase()}</span>
          </div>
        </div>
        <div class="flex-1 min-w-0">
          <h3 class="text-white font-bold text-sm sm:text-base leading-tight truncate uppercase tracking-wide" title="${agent.name}">${agent.name}</h3>
        </div>
      </div>
      ...
      <div class="bg-black/20 rounded-xl p-2 border border-white/10 flex items-center justify-between shadow-[inset_0_1px_3px_rgba(0,0,0,0.2)]">
        ...
        <div class="text-xl sm:text-2xl font-heading text-cyan-400 font-black drop-shadow-[0_0_8px_rgba(34,211,238,0.4)]">${agent.stats.total}</div>
      </div>
...
async function updateDashboard() {
  ...
  grid.innerHTML = globalAgents.map(agent => renderAgentCard(agent)).join('');
  ...
  const totals = globalAgents.reduce((acc, agent) => {
    ...
    return acc;
  }, { total: 0, ros: 0, towing: 0, assigned: 0, dealer: 0, failed: 0, ongoing: 0, escalation: 0, cancelled: 0, postponed: 0, closed: 0, preclose: 0 });
  ...
  document.getElementById('kpi-ongoing').textContent = totals.ongoing;
  document.getElementById('kpi-escalation').textContent = totals.escalation;
  document.getElementById('kpi-cancelled').textContent = totals.cancelled;
  document.getElementById('kpi-postponed').textContent = totals.postponed;
  document.getElementById('kpi-closed').textContent = totals.closed;
  document.getElementById('kpi-preclose').textContent = totals.preclose;
